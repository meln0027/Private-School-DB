...
--for student table
CREATE TYPE student_status AS ENUM ( 'active', 'graduated', 'withdrawn', 'transferred', 'inactive' );
CREATE TYPE student_gender AS ENUM ( 'unknown', 'm', 'f', 'x');

--for person_address table
CREATE TYPE person_type AS ENUM ( 'student', 'guardian', 'employee', 'other' );
CREATE TYPE address_type AS ENUM ( 'home', 'guardian', 'work', 'mailing', 'other' );

--for student_guardian table
CREATE TYPE relationship AS ENUM ( 'guardian', 'mother', 'father', 'family member', 'other' );

--for semester table
CREATE TYPE semester_name AS ENUM ( 'fall', 'winter', 'spring', 'summer' );

--for service table
CREATE TYPE service_type AS ENUM ( 'academic_year', 'academic_semester', 'academic_month', 'academic_other', 'service_transportation', 'service_lunch', 'service_other' );
CREATE TYPE billing_period AS ENUM ( 'year', 'semester', 'month', 'week', 'one_time_payment', 'other' );

--for invoice table
CREATE TYPE customer_type AS ENUM ( 'guardian', 'student', 'employee', 'sponsor', 'other' );
CREATE TYPE payment_status AS ENUM ( 'sent', 'pending', 'paid', 'cancelled' );

--for invoice_detail table
CREATE TYPE period AS ENUM ( 'year', 'semester', 'month', 'week', 'one_time_payment', 'other' );

--for payment_detail table
CREATE TYPE payment_method AS ENUM ( 'cash', 'card', 'online', 'check', 'mobile', 'other' );
...
CREATE TABLE IF NOT EXISTS student (
    ...
    gender student_gender NOT NULL DEFAULT 'unknown',
...
CREATE TABLE IF NOT EXISTS customer (
    id INT PRIMARY KEY NOT NULL,
    customer_type customer_type NOT NULL DEFAULT 'guardian',
    student_id INT,
    guardian_id INT,
    employee_id INT,
    ...
    CONSTRAINT fk_student_customer FOREIGN KEY (student_id) REFERENCES student(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_guardian_customer FOREIGN KEY (guardian_id) REFERENCES guardian(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_employee_customer FOREIGN KEY (employee_id) REFERENCES employee(id) ON DELETE CASCADE ON UPDATE CASCADE
);

DROP TABLE IF EXISTS person_address;
CREATE TABLE IF NOT EXISTS person_address (
    id INT PRIMARY KEY NOT NULL,
    person_type person_type DEFAULT 'student',
    ...
    CONSTRAINT fk_student_person_address FOREIGN KEY (student_id) REFERENCES student(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_guardian_person_address FOREIGN KEY (guardian_id) REFERENCES guardian(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_employee_person_address FOREIGN KEY (employee_id) REFERENCES employee(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_customer_person_address FOREIGN KEY (customer_id) REFERENCES customer(id) ON DELETE CASCADE ON UPDATE CASCADE
);  
...
CREATE TABLE IF NOT EXISTS semester (
    id INT PRIMARY KEY NOT NULL,
    name semester_name DEFAULT 'fall',
...
CREATE TABLE IF NOT EXISTS service (
    ...
    semester_id INT,
    academic_year_id INT,
    service_type service_type NOT NULL DEFAULT 'academic_year',
    billing_period billing_period NOT NULL DEFAULT 'year',
    price DECIMAL(10,2),
    description VARCHAR(250),
...
CREATE TABLE IF NOT EXISTS discount (
    ...
    value DECIMAL(10,2),
    description VARCHAR(250),
    discount_type discount_type NOT NULL DEFAULT 'fixed amount'
);
...
CREATE TABLE IF NOT EXISTS invoice (
    ...
    description VARCHAR(250),
    CONSTRAINT fk_customer_invoice FOREIGN KEY (customer_id) REFERENCES customer(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS payment (
    ...
    description VARCHAR(250),
    CONSTRAINT fk_customer_payment FOREIGN KEY (customer_id) REFERENCES customer(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS invoice_tax (
    ...
    CONSTRAINT fk_invoice_invoice_tax FOREIGN KEY (invoice_id) REFERENCES invoice(id) ON DELETE CASCADE ON UPDATE CASCADE,
